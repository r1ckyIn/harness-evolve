...
- [ ] **SCN-01**: `harness-evolve init` 扫描用户现有的 CLAUDE.md、.claude/rules/、settings.json、.claude/commands/ 配置，生成配置质量报告
- [ ] **SCN-02**: 深度扫描检测冗余规则（多个文件定义相同约束）、缺失机械化（应为 hook 但写在 rules 里的操作）、过时配置（引用不存在的文件或命令）
